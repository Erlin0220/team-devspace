...
Team DevSpace serves `/admin*` from the existing Gateway Worker and protects that path with a project-owned Cloudflare Access self-hosted Application. The Admin Web adapter and the existing `ADMIN_TOKEN` CLI adapter both call the same Admin Service, which remains the only owner of Access Key and Device Binding lifecycle transitions, Tunnel/DNS cleanup, fail-closed behavior and reconciliation.
