...
`status` performs a bounded query and exits; the agent can inspect it again while doing other work. There is no daemon, background delivery promise or long-held browser/DevSpace request. Build IDs and an unresolved submission marker are saved under ignored `build/codemagic/<commit>.json`. The shared SQLite-backed process lock prevents concurrent local submissions and is released automatically if the local process exits or crashes. Cloud build status remains authoritative.
