...
`.github/workflows/build-installers.yml` is intentionally layered. Shared source checks, dependency audit, gateway/runtime tests and deploy dry-run run once on Linux before native packaging. The three enabled native package jobs then run in parallel and execute only their platform-specific package/installer checks; the expensive Windows installer transaction runs only for an actual publication. Matrix fail-fast is enabled so one native failure cancels the remaining package work. Published Windows installers must carry the expected fixed internal Authenticode signer, but CI does not mutate the runner's Windows trust stores: it verifies the embedded signer and Authenticode integrity, while actual current-user trust is established only by the shipped trust helper on the employee machine. Apple credentials remain optional; when complete they enable app/PKG signatures plus notarization/staple, and when absent the job explicitly reports unsigned/unnotarized output.
