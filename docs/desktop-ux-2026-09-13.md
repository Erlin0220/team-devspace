...
macOS reopening an unchanged installation starts existing LaunchAgent jobs rather than recreating all startup entries. Missing/broken jobs still fall through to the existing verified bootstrap recovery. AppKit renders the same one-level submenu and remains a thin frontend.
