...
import { loadState, stateHome, atomicJson, approvedRoots } from '../client/state.mjs';
...
const root = (await approvedRoots([values.root ?? state.roots[0]]))[0];
if (!state.roots.includes(root)) throw new Error('Use one of the Device\'s configured Allowed Roots');
...
const insideRoot = state.roots.some(allowed => {
  const part = relative(allowed, outsideDirectory);
  return part === '' || (!part.startsWith('..') && !isAbsolute(part));
});
if (insideRoot) throw new Error('Temporary directory is inside an Allowed Root; choose a narrower project directory before preparing the boundary acceptance fixture');
