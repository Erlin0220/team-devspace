...
const app = { id: applicationId, name: ACCESS_APP_NAME, type: 'self_hosted',
  domain: `${hostname}/admin*`, destinations: [{ type: 'public', uri: `${hostname}/admin*` }] };
const policy = { id: policyId, name: ACCESS_POLICY_NAME, decision: 'allow',
  include: [{ email: { email: emails[0] } }] };

function apiFixture({ applications = [], policies = [], fail } = {}) {
  const calls = [];
  const api = async (path, method = 'GET', body) => {
    calls.push({ path, method, body });
    if (fail) throw new Error('cloudflare unavailable');
    if (path.endsWith(`/access/apps/${applicationId}`)) return app;
    if (path.includes('/access/apps?')) return applications;
    if (path.endsWith('/access/apps') && method === 'POST') return app;
...
test('post-deploy admin probe accepts only a challenge/deny and rejects exposed HTML', async () => {
  ...
  await assert.rejects(verifyAdminProtection('https://team.example.test', async () => new Response(
    JSON.stringify({ error: 'access_required' }), { status: 403, headers: { 'X-Team-Request-Id': 'worker-request' } })),
  /not fail-closed/);
});
